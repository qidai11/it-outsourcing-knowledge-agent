...
async def test_real_ragflow_alpha_query_does_not_return_beta() -> None:
    ...
    async with httpx.AsyncClient(base_url=base_url, timeout=30.0) as http:
        adapter = RagflowAdapter.from_http_client(http, api_key=api_key, object_store=store)
        ...
        try:
            ...
            assert chunks
            assert all(chunk.project_id == "PRJ-RETAIL-ALPHA" for chunk in chunks)
            assert all(
            ...
            await adapter.delete_document(
                DeleteKnowledgeDocumentRequest(
                    "PRJ-RETAIL-ALPHA", alpha_version, alpha_space.knowledge_space_id
                )
            )
            await adapter.delete_document(
                DeleteKnowledgeDocumentRequest(
                    "PRJ-LOGISTICS-BETA", beta_version, beta_space.knowledge_space_id
                )
            )
