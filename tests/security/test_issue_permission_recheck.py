...
@pytest.mark.asyncio
async def test_viewer_cannot_create_even_after_confirming() -> None:
    ...
    auth_repo.memberships[(user_id, project_id)] = MembershipAccessRecord(
        ...
        project_code="PRJ-RETAIL-ALPHA", role=ProjectRole.VIEWER,
    ...
    draft = await drafts.create_from_text(
        run_id=uuid4(), project_id=project_id, created_by=user_id, text="upload failed"
    )
    ...
    with pytest.raises(IssueCreationDenied, match="role"):
