...
@pytest.mark.asyncio
async def test_mutating_draft_after_confirmation_invalidates_confirmation() -> None:
    ...
    repo.drafts[draft.id] = repo.drafts[draft.id].model_copy(update={"title": "tampered title"})
    ...
    with pytest.raises(IssueCreationDenied, match="does not match"):
        await creation.execute(draft_id=draft.id, confirmation_id=receipt.id, actor_id=user_id)
