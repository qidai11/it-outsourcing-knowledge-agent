...
@pytest.mark.asyncio
async def test_issue_create_metrics_distinguish_created_and_replay() -> None:
    ...
    auth_repo.memberships[(user_id, project_id)] = MembershipAccessRecord(
        company_id=uuid4(), client_id=uuid4(), project_id=project_id,
        project_code="PRJ-METRIC", role=ProjectRole.DEVELOPER,
        valid_from=now, valid_to=None,
    )
    repo = FakeIssueWorkflowRepository(clock=lambda: now)
    drafts = IssueDraftService(repo)
    draft = await drafts.create_from_text(
        run_id=uuid4(), project_id=project_id, created_by=user_id, text="metric issue"
    )
    confirmations = IssueConfirmationService(repo, drafts=drafts, clock=lambda: now)
    prompt = await confirmations.prepare(draft.id)
    receipt = await confirmations.record_decision(
    ...
    with metrics_context(metrics):
        await service.execute(draft_id=draft.id, confirmation_id=receipt.id, actor_id=user_id)
        await service.execute(draft_id=draft.id, confirmation_id=receipt.id, actor_id=user_id)
    rendered = metrics.render_latest().decode()
    assert 'project_agent_issue_create_total{outcome="created"} 1.0' in rendered
    assert 'project_agent_issue_create_total{outcome="already_created"} 1.0' in rendered
