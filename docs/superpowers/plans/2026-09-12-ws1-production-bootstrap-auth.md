...
- [ ] **Step 1: Write the failing unit tests for role-derived document permissions**
...
- [ ] **Step 3: Implement the minimal document access policy**
...
- [ ] **Step 4: Run Task 1 tests and existing authorization/security regression**
...
- [ ] **Step 5: Run static checks for the new application service**
...
- [ ] **Step 1: Write failing runtime/lifespan tests**
...
- [ ] **Step 3: Implement `build_api_runtime()`**
...
- [ ] **Step 4: Replace app-state test shortcuts with the runtime lifespan seam in `create_app()`**
...
- [ ] **Step 1: Add deterministic JWT test helper and failing API authentication tests**
...
- [ ] **Step 3: Implement the FastAPI runtime/session/authentication dependencies**
...
- [ ] **Step 1: Write the failing offline and live document authorization tests before changing routes**
...
- [ ] **Step 3: Add the minimal Task 6 commit barrier required by production transactions**
...
- [ ] **Step 4: Replace the document service app-state shortcut with request-scoped production construction**
...
- [ ] **Step 5: Add membership/operation authorization to every document route**
...
- [ ] **Step 6: Run the offline document/auth/lifecycle GREEN gate**
...
- [ ] **Step 7: Run the real PostgreSQL revocation GREEN gate**
...
- [ ] **Step 8: Update status documentation without overstating evidence**
...
- [ ] **Step 10: Verify WS1 scope isolation before commit**
...
```text
[ ] create_app() remains import-safe and testable through runtime_factory/dependency overrides.
[ ] FastAPI lifespan constructs and closes API-facing production resources.
[ ] One request uses one transactional AsyncSession for authorization/document repositories.
[ ] Missing/invalid/expired Bearer JWT -> 401 + Bearer challenge.
[ ] JWT output contains only user identity; forged role/project claims grant nothing.
[ ] Current PostgreSQL membership is loaded for every protected document request.
[ ] Viewer/non-member/expired/revoked/wrong-project/wrong-company access -> 403.
[ ] Same still-valid JWT is denied on the request immediately after membership revocation.
[ ] DocumentActor is built server-side from authenticated user + current membership-derived permissions.
[ ] app.state.document_actor and request.state.document_actor are absent from production document routing.
[ ] Existing Task 6 document lifecycle tests remain green.
[ ] No Run API, worker execute/resume handler, real LLM, QA/Issue Graph change, observability, Docker, or evaluation code is introduced.
[ ] No prohibited architecture component is added.
