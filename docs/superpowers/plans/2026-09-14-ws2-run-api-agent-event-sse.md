...
### Task 1: Freeze the Run/Event Domain Contract and Correct the Run Schema
...
### Task 2: Add the Run Repository and Same-Session Job Enqueue Boundary
...
### Task 3: Implement Authorized Run Creation and Durable Initial Input
...
### Task 4: Add `POST /runs` and Run Read HTTP APIs with Current-Membership Security
...
### Task 5: Add Durable Agent Event SSE with Ordering and Reconnect
...
### Task 6: Add Durable Resume Input and `POST /runs/{run_id}/resume`
...
### Task 7: Prove PostgreSQL Atomicity, Project Security, Revocation, and End-to-End WS2 Persistence
...
```text
[ ] WS1 runtime/auth/session foundation remains intact.
[ ] POST /api/v1/runs requires explicit project_id and current membership.
[ ] Run business mode is durably stored for every new WS2 Run.
[ ] New Runs begin QUEUED with started_at NULL.
[ ] New or supplied Thread is validated against durable project/company/user ownership.
[ ] Run + initial RUN_QUEUED event + EXECUTE_AGENT_RUN job commit atomically.
[ ] Query text is durable outside background_jobs.
[ ] GET /api/v1/runs/{run_id} re-authorizes current project membership.
[ ] Safe result_ref/summary projection never returns arbitrary provider payloads.
[ ] Agent Events are append-only and have unique per-run sequence numbers.
[ ] SSE uses sequence_no as id and supports Last-Event-ID replay.
[ ] SSE ordering/reconnect/terminal/disconnect semantics are tested.
[ ] SSE cross-project access is denied.
[ ] Resume requires current project membership plus original Run actor identity.
[ ] Resume requires WAITING_CONFIRMATION + matching payload hash.
[ ] Resume input is durable in RUN_RESUME_QUEUED event.
[ ] Resume transition + RESUME_AGENT_RUN enqueue is one transaction.
[ ] Duplicate/replayed resume cannot create a second event/job.
[ ] No WS3 Worker execute/resume handler is implemented.
[ ] No LangGraph execution occurs from an API request.
[ ] No real LLM/retrieval/Issue Evidence/observability/Docker/evaluation scope is pulled into WS2.
[ ] Offline WS2 gate passes.
[ ] Live PostgreSQL WS2 gate passes with 0 skipped tests before live durability is claimed.
